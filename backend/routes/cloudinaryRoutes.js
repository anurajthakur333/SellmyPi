...
router.post('/delete', async (req, res) => {
  try {
    ...
    console.log('Received delete request:', { public_id, imageUrl });
    ...
    if (!finalPublicId) {
      console.error('Invalid public_id or imageUrl:', { public_id, imageUrl });
    ...
    if (urlCloudName && urlCloudName !== currentConfig.cloud_name) {
      console.error('Cloud name mismatch:', {
        urlCloudName,
    ...
    console.log('Attempting to delete image with public_id:', finalPublicId);
    console.log('Using Cloudinary cloud_name:', currentConfig.cloud_name);
    ...
    if (result.result === 'ok') {
      ...
      console.warn('Image not found in Cloudinary:', finalPublicId);
      res.status(404).json({ 
        success: false, 
